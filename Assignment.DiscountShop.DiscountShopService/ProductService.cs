...
namespace Assignment.DiscountShop.DiscountShopService
{
    public class ProductService : IProductService
    {
        ...
        public Product CreateProduct(string name, string description, decimal costPerUnit, ProductCategory category)
        {
            int maxId = products.Select(c => c.Id)
                .DefaultIfEmpty(0).Max();
            products.Add(new Product(++maxId, name, description, costPerUnit, category));
